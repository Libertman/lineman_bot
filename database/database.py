...
deadlines = {'Физическая культура':
             [Deadline(name='Аттестация по модулю: Основы теории физической культуры', deadline=datetime(2024, 10, 6), subject='Физическая культура'),
              Deadline(name='Аттестация по модулю: Медико-биологические основы физической культуры', deadline=datetime(2024, 10, 13), subject='Физическая культура'),
...
nearest_list_deadlines = list(dropwhile(lambda x: x.deadline.timestamp() <= datetime.now(timezone('Europe/Moscow')).timestamp(), sorted(reduce(lambda x, y: x + y, [values for values in deadlines.values()]),key=lambda x: x.deadline)))